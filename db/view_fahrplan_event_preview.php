...
*/
class View_Fahrplan_Event_Preview extends View {
  ...
  */
  ...
  {
    $this->table = "event, event_state, conference, room";
    $this->domain = "event";
    $this->order = "lower(event.title), lower(event.subtitle)";
    $this->join = "event.event_state_id = event_state.event_state_id AND event_state.tag IN ('confirmed','accepted') AND event.start_time IS NOT NULL and event.room_id IS NOT NULL AND event.day IS NOT NULL AND event.conference_id = conference.conference_id AND event.room_id = room.room_id";
