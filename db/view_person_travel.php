...
class View_Person_Travel extends View
{
  ...
  public function select( $where )
  {
    ...
    $sql = "SELECT person_id, name, conference_id, arrival_transport_id AS transport_id, arrival_from AS from, arrival_to AS to, arrival_number AS number, arrival_date AS date, arrival_time AS time, f_arrival_pickup AS f_pickup FROM person_travel INNER JOIN view_person USING (person_id) WHERE conference_id = '{$conference_id}' AND f_arrival_pickup = 't' UNION SELECT person_id, name, conference_id, departure_transport_id AS transport_id, departure_from AS from, departure_to AS to, departure_number AS number, departure_date AS date, departure_time AS time, f_departure_pickup AS f_pickup FROM person_travel INNER JOIN view_person USING (person_id) WHERE conference_id = '{$conference_id}' AND f_departure_pickup = 't' ORDER BY date,time;";
