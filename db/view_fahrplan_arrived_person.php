...
class View_Fahrplan_arrived_Person extends View
{
  public function __construct($select = array())
  {
    $this->distinct = "person_id";
    $this->table = "event_person, person, event_role, event_role_localized, event, event_state, event_role_state, person_travel";
    $this->domain = "person";
    ...
    $this->join = "event.event_state_id = event_state.event_state_id AND event_state.tag = 'confirmed' AND event_person.event_id = event.event_id AND event_person.person_id = person.person_id AND event_person.event_role_id = event_role.event_role_id AND event_person.event_role_id = event_role_localized.event_role_id AND event_role_localized.language_id = {$this->get_language_id()} AND event_person.event_role_state_id = event_role_state.event_role_state_id AND event_role_state.tag = 'confirmed' AND event.f_public = TRUE AND person.person_id = person_travel.person_id AND event.conference_id = person_travel.conference_id";
    ...
    $this->field['f_arrived']['type'] = 'VARCHAR';
    $this->field['f_arrived']['table'] = 'person_travel';
