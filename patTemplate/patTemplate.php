 ...
 * patTemplate
 *
 * $Id: patTemplate.php,v 1.94.4.5 2004/11/23 13:50:57 schst Exp $
 *
...
class patTemplate
{
  ...
  */
  function addVar( $template, $varname, $value )
  {
    ...
    for( $i = 0; $i < $cnt; $i++ )
    {
      ...
      if (!isset($value[$i])) {
        trigger_error("Unset var: $varname i= $i\n in template $template.\n");
        continue;
      }
  ...
  */
  ...
  {
    $allowed_functions = array('nl2br');
    foreach($vars as $varname => $value) {
      if (isset($this->_templates[$template]['modifyVars'][$varname])) {
        // a modifier is set
        $modifier = $this->_templates[$template]['modifyVars'][$varname];
        if (in_array($modifier['mod'], $allowed_functions) && is_callable($modifier['mod'])) {
          $vars[$varname] = call_user_func($modifier['mod'], htmlspecialchars($value));
        } else { 
        ...
        if (is_array($value)) {
          // if this is an array than we will get here again when we process the real value
          continue;
        } else {
          // apply default modifier
          $vars[$varname] = htmlspecialchars($value);
        }
      }
    }
    return true;
