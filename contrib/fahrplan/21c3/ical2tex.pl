...
while (<FAHRPLAN>) {
	...
	$title =~ s/\Ã\¼/ü/g; # actually bugs in input of uid 99
	$title =~ s/\Ã\¤/ä/g;
	$title =~ s/Großprojekten/Groß\\-pro\\-jek\\-ten/g;
	...
	$subtitle =~ s/\Ã\¼/ü/g; # actually bugs in input of uid 99, 211
	$subtitle =~ s/\Ã\¤/ä/g;
	...
	$description =~ s/\Ã\¼/ü/g; # actually bugs in input of uid 99
	$description =~ s/\Ã\¤/ä/g;
	$description =~ s/\Ã\¶/ö/g;
	$description =~ s/_/\\_/g;
	$description =~ s/\[/\\type{\[}/g;
	...
	if ($duration == 0.5) { # kleine Blöcke sind mit obiger Berechnung zu lang
	    $length = 150 - length($title) - length($subtitle) - length($speaker)
	}
	if ($uid == 278) { # enthält viele Üs, so dass die Zeilen zu gross sind
