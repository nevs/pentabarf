module Momomoto
  class View_conflict_event_person < Base
    def initialize
      super
      @domain = 'conflict'
      @order = 'conflict_id, title, name'
      @query = "SELECT conflict_event_person.conflict_id, 
                       ...
                       event.title,
                       view_person.name
                  FROM conflict_event_person(%conference_id%) 
