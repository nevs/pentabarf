  ...
  class View_find_person < Base
    def initialize
      ...
      @fields = {
        ...
        :s_conference => Datatype::Keysearch.new( {:virtual=>true,:key_field=>'person_id',:subselect=>"SELECT person_id FROM event_person INNER JOIN event USING(event_id) WHERE event.conference_id IN (%%%)"} ),
        :s_event_role => Datatype::Keysearch.new( {:virtual=>true,:key_field=>'person_id',:subselect=>"SELECT person_id FROM event_person WHERE event_person.event_role_id IN (%%%)"} ),
