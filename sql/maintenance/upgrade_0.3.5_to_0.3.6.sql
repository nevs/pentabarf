...
CREATE TABLE base.account ( account_id SERIAL, login_name TEXT NOT NULL UNIQUE, email TEXT, salt TEXT, password TEXT, edit_token TEXT, current_language TEXT NOT NULL DEFAULT 'en', current_conference_id INTEGER, preferences TEXT, last_login TIMESTAMP, person_id INTEGER, CHECK (login_name <> 'logout'), CHECK ( strpos( login_name, ':' ) = 0 ));
...
CREATE TABLE base.event ( event_id SERIAL NOT NULL, conference_id INTEGER NOT NULL, tag TEXT, title TEXT NOT NULL, subtitle TEXT, conference_track TEXT, conference_team TEXT, event_type TEXT, duration INTERVAL NOT NULL DEFAULT '1:00:00', event_origin TEXT NOT NULL DEFAULT 'idea', event_state TEXT NOT NULL DEFAULT 'undecided', event_state_progress TEXT NOT NULL DEFAULT 'new', language TEXT, conference_room TEXT, day SMALLINT, start_time INTERVAL, abstract TEXT, description TEXT, resources TEXT, public BOOL NOT NULL DEFAULT FALSE, paper BOOL, slides BOOL, remark TEXT, submission_notes TEXT);
CREATE TABLE event (
  FOREIGN KEY (conference_id) REFERENCES conference (conference_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE log.event_feedback () INHERITS( base.logging, base.event_feedback );
