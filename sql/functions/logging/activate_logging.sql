...
CREATE OR REPLACE FUNCTION logging.activate_logging() RETURNS VOID AS $$
  ...
  BEGIN
    ...
    LOOP
      ...
      IF ( NOT EXISTS( SELECT 1 FROM information_schema.triggers WHERE trigger_name = trigname AND event_object_schema = 'public' AND event_object_table = tablename ) ) THEN
