...
CREATE OR REPLACE FUNCTION account_forgot_password(integer, char(64)) RETURNS INTEGER AS $$
  ...
  BEGIN
    ...
    SELECT INTO cur_person person_id FROM password_reset WHERE person_id = cur_person_id;
    IF FOUND THEN
      RAISE EXCEPTION 'This account has already been reset in the last 24 hours.';
    END IF;
    INSERT INTO password_reset(person_id, activation_string, password_reset) VALUES (cur_person_id, cur_activation_string, now());
