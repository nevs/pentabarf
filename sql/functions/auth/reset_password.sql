...
CREATE OR REPLACE FUNCTION auth.reset_password( reset_string CHAR(64), new_password TEXT ) RETURNS INTEGER AS $$
  DECLARE
    ...
    binary_salt BYTEA;
  BEGIN
    SELECT INTO cur_account_id account_id FROM auth.account_password_reset WHERE activation_string = reset_string;
    IF NOT FOUND THEN
    ...
    DELETE FROM auth.account_password_reset WHERE reset_time < now() + '-1 day' OR activation_string = reset_string;
