...
CREATE OR REPLACE FUNCTION account_forgot_password(integer, char(64)) RETURNS INTEGER AS $$
  DECLARE
    ...
    cur_person RECORD;
  BEGIN
    DELETE FROM password_reset WHERE password_reset < now() + '-1 day';
    ...
    END IF;
    INSERT INTO auth.password_reset(person_id, activation_string, password_reset) VALUES (cur_person_id, cur_activation_string, now());
