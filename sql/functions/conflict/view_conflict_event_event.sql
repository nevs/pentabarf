CREATE OR REPLACE FUNCTION conflict.view_conflict_event_event( conference_id INTEGER ) RETURNS SETOF conflict.view_conflict_event_event AS $$
  SELECT conflict_event_event.conflict,
         conflict_localized.name AS conflict_name,
         conflict_level_localized.conflict_level,
         ...
         event1.title1,
         event2.title2
    FROM conflict.conflict_event_event( $1 )
